...
//   $Game::DefaultCameraDataBlock = "GameCameraData";
...
function GameCore::onClientEnterGame(%game, %client)
{
   ...
   %game.preparePlayer(%client);
    ...
    %client.gameCamera = new PathCamera("gCamera")
	{
		dataBlock = GameCameraData;
	};

	MissionCleanup.add(%client.gameCamera);
	%client.gameCamera.scopeToClient(%client);
}
